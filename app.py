...
with col3:
    league_type = st.selectbox("League / Competition", 
        ["English Premier League", "Spanish La Liga", "Italian Serie A", "German Bundesliga", "NPFL", "Champions League", "Europa League", "World Cup"])
    m_seriousness = st.selectbox("Match Intensity / Category", 
        ["Friendly", "Normal League Game", "Local Derby", "Relegation Battle", "Title Decider", "Cup Final"])
    h2h_data = st.text_area("H2H History", placeholder="Previous meetings...", height=65)
    form_data = st.text_area("Form History", placeholder="Last 5 results...", height=65)
...
if st.button("🚀 EXECUTE 10,000 RUN SIMULATION"):
    h_base = (h_sot * 0.18) + (h_bc * 0.40) + (h_pos * 0.005)
    a_base = (a_sot * 0.18) + (a_bc * 0.40) + (a_pos * 0.005)
    
    m_int = {"Friendly": 0.8, "Normal League Game": 1.0, "Local Derby": 1.2, "Relegation Battle": 1.15, "Title Decider": 1.3, "Cup Final": 1.4}[m_seriousness]
    
    h_final = (h_base * (1 - (h_pen * 0.12)) * (1/a_def)) * m_int
    a_final = (a_base * (1 - (a_pen * 0.12)) * (1/h_def)) * m_int
    ...
    p_gg, p_ov25 = np.mean((h_sim > 0) & (a_sim > 0)), np.mean((h_sim + a_sim) > 2.5)
    p_h3 = np.mean(h_sim >= 3) # 3+ Streak Home
    p_a3 = np.mean(a_sim >= 3) # 3+ Streak Away
    p_corn = (h_sot + a_sot + (h_final + a_final)) * 1.5 # Corner Projection logic
    
    data = [
        {"Market": "HOME WIN", "Prob": p1, "Bookie": bk_1},
        {"Market": "DRAW", "Prob": px, "Bookie": bk_x},
        {"Market": "AWAY WIN", "Prob": p2, "Bookie": bk_2},
        {"Market": "GG (BTTS)", "Prob": p_gg, "Bookie": bk_gg},
        {"Market": "OVER 2.5", "Prob": p_ov25, "Bookie": bk_ov},
