...
def predict(model, features: np.ndarray, model_name: str = "model") -> Tuple[float, Optional[str]]:
    if model is None: return 0.0, f"{model_name} not loaded"
    try:
...
def load_models():
    models = {}
    try: models["goal"] = joblib.load("model.pkl")
    except: models["goal"] = None
    try: models["corner"] = joblib.load("corner_model.pkl")
    except: models["corner"] = None
    return models
...
if submit:
    all_errors = validate_team_stats(h_stats, h_name) + validate_team_stats(a_stats, a_name)
    if all_errors or models["goal"] is None:
        st.error("Error: Check inputs or model.pkl presence.")
        st.stop()

    h_f, a_f = f_score(h_form)[0], f_score(a_form)[0]
    h_b, a_b = player_attack_boost(h_xi)[0], player_attack_boost(a_xi)[0]
