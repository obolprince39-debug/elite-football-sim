    ...
    try:
        ...
        for team in data['standings'][0]['table']:
            if team_name.lower() in team['team']['name'].lower():
                # SAFETY CHECK: Handle cases where 'form' might be None from the API
                raw_form = team.get('form')
                clean_form = raw_form.replace(',', '') if raw_form else "WWDLW"
...
def validate_team_stats(stats: Dict, team_name: str) -> List[str]:
    ...
    if stats.get("gpg", 0) > 5: 
        errors.append(f"{team_name}: Goals/game seems unrealistic")
    return errors
